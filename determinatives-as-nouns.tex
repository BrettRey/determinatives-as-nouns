...
\newcommand{\synnode}[2]{\shortstack{\footnotesize #1\\#2}}
...
\begin{abstract}
English determinatives are nouns: a fourth subcategory of Noun beside common nouns, proper nouns, and pronouns. I compare the grammatical profile of the determinative inventory of \textit{The Cambridge Grammar of the English Language} with the three established subcategories and with adjectives. Quantificational common nouns share partitives, number transparency, and restricted dependents with the quantifiers; demonstratives inflect for number; compounds take head genitives. The adjectival profile of grade and degree modification is confined to \mention{few}, \mention{many}, \mention{much}, and \mention{little}. The articles and \mention{every} belong through their integration into the determinative system. In independent uses (\mention{take some}, \mention{the lucky few}) I replace the grammar's fusion of determiner and Head functions with ordinary Head relations: the determinative heads a nominal that heads the noun phrase. An audit of four implementations shows that the account's economy is the economy of treating determinatives as nouns in the syntax: a separate category matches it only with a disjunct in its category condition and a category that duplicates Noun's profile. The grouping also unifies three secondary uses the grammar states separately and reanalyses its complex determinatives \mention{a few}, \mention{a little}, \mention{a great many}, and \mention{many a}. The noun phrase stays noun-headed; this isn't the DP hypothesis.
\end{abstract}
...
I adopt the general framework of \textit{The Cambridge grammar of the English language} (\textit{CGEL}; \citealp{huddleston2002}). Unlike \textit{CGEL}, though, I include determinatives within Noun. The claims concern synchronic English lexical categories; examples without a source are constructed, and attested examples name theirs.\footnote{I treat these categories as language-specific: a categorization supported by another language's grammar doesn't determine the English categorization.}
...
\begin{table}[H]
\centering\small
\caption{Four combinations of categorization and Head analysis. The proposed analysis is the first row. All keep \mention{apples} as the ultimate lexical head of \mention{some apples}.}\label{tab:accounts}
...
Earlier accounts connect articles with pronouns or give both nominal structure, at different analytical levels (Appendix~\ref{sec:historical}); \textcite{vaneynde2003determiner} argues from inflection that determiners are heterogeneous, some adjectives and some nouns (§\ref{sec:four-questions}). What's new here is the scope and the pairing: the full inventory of \textit{CGEL}'s determinative category, articles included, as one coordinate subcategory of Noun; ordinary Head for its independent uses; and an explicit comparison of the four combinations of taxonomy and Head analysis, through a matched fragment and a coverage audit. Hudson's nested noun taxonomy is considered in §\ref{sec:inheritance}; Spinillo's restricted article grouping in §\ref{sec:articles}.
...
With \mention{a few}, modifier position distinguishes two attachment sites. \textit{CGEL} contrasts internal \mention{very} in \mention{a very few mistakes} with peripheral \mention{quite} in \mention{quite a few mistakes} \citep[p.~392, (61)]{huddleston2002}; the reversed orders \ungram{\mention{very a few}} and \ungram{\mention{a quite few}} are excluded. On the analysis of §\ref{sec:article-membership}, \mention{a few} is an NP with \mention{a} in Det and \mention{few} as head, so \mention{very} modifies the head and \mention{quite} modifies the NP, as in \mention{quite a mess}.
...
Case gives a further partial connection. Genitives occur across the proposed subcategories: \mention{dog's}, \mention{Kim's}, \mention{my}, and \mention{someone's}. Personal pronouns have fuller case paradigms. Most determinatives lack case inflection. The compounds' genitives belong to the NP: \textit{CGEL} lists \mention{everyone's} beside \mention{Edward's} among head genitives, marked by \enquote{the inflection of the head noun}, and \mention{everyone else's} and \mention{somebody local's} beside \mention{the King of England's} among phrasal genitives, with the suffix after the postmodifier \citep[479]{huddleston2002}. On \textit{CGEL}'s categorization the head of \mention{everyone's} is a determinative, so that definition is stretched at this point; within Noun it isn't. This supports the compounds' nominal affinity more directly than the category's membership as a whole.
...
Table~\ref{tab:existing} summarizes the range within each category; individual lexemes differ in which properties they exhibit. Functions concern phrases, inflection concerns word forms, and inventory closure concerns categories. AdjP and AdvP denote adjective and adverb phrases. The modifiers row records which modifiers occur; where they attach is the analysis of §\ref{sec:payne}.

Proper names permit nominal premodifiers such as \mention{architect Norman Foster} \citep[517--518]{huddleston2002}.

\begin{table}[H]
\centering\small
\caption{Profiles compared, in three blocks. Interrogative and relative forms follow \textit{CGEL}'s inventory.}\label{tab:existing}
...
\midrule
\multicolumn{5}{l}{\itshape Functions and constructions} \\
...
\addlinespace
\multicolumn{5}{l}{\itshape Dependents} \\
...
Modifiers admitted & Productive AdjP and nominal premodifiers, including determinative-headed phrases; relatives; peripheral \mention{only the book} & Restricted AdjP and nominal premodifiers; embellishments; peripheral \mention{even Kim} & Restricted AdjP premodifiers and relative postmodifiers; peripheral \mention{only you} & Degree AdvPs on the four gradable quantifiers; approximatives (\mention{almost every}, \mention{hardly any}, \mention{almost ten}); NP and determinative-headed degree modifiers; adjectives after an external determiner (\mention{the lucky few}); relatives \\
...
\addlinespace
\multicolumn{5}{l}{\itshape Form, meaning, and inventory} \\
...
The nominal and adjectival comparisons each involve connected properties. On the noun side, quantity combines with selected complements, number transparency, and restricted dependents. On the adjective side, grade combines with degree modifiers and comparative complements. The four gradable quantifiers' limited number isn't itself a reason to discount their adjectival pattern.

The nominal connections recur across more of the determinative system, in more constructions and for more lexeme groups, as the claim register records lexeme by lexeme. Partitives link \mention{many} with \mention{some}; relative postmodification links \mention{few} with demonstratives and compounds. Determination and reference connect determinatives with genitive NPs and pronouns, while demonstrative number supplies a morphological parallel. These links recur across constructions whose conditions were specified separately. The gradable quantifiers participate in this wider range alongside their adjectival pattern.
...
\textit{CGEL}'s treatment of \mention{my} supplies a precedent for a restricted noun form \citep[470--471]{huddleston2002}.\footnote{\textcite{lenchukahmed2014determinatives} objected to \textcite{reynolds2013determiners} that its determinative tests make \mention{my}, \mention{your}, and \mention{his} determinatives, and that the partitive test admits \mention{few}, \mention{many}, and \mention{several} while their count and number restrictions contradict it; \textcite{reynolds2014determinatives} replied. Within Noun the first objection concerns the boundary between two subcategories, and the paradigm decides it: \mention{my} belongs with \mention{I}, \mention{me}, and \mention{mine} by person, number, and case, and that organization outweighs the functional diagnostics for this internal boundary, as \textcite{reynolds2014determinatives} argued; the partitive's restrictions are the lexical target conditions of §\ref{sec:fragment}, which no single test carries.} Dependent \mention{my} and independent \mention{mine} belong to one pronoun paradigm. \mention{My} remains a noun even though, in the ordinary NP construction, it requires a following nominal. The articles lack an independent argument form; their positive evidence is integration into determinative, and their nounhood depends on the category-level argument. The inference has two stages. The first places the articles within the determinative category: they fill the Det position the unrestricted members fill, take part in its contrasts of definiteness, quantity, and count selection, and sit inside the determinative cluster of the 2021 matrix (§\ref{sec:inheritance}). The second places that category within Noun by the profile of §\ref{sec:proposal}. The criterion for a restricted member is the first stage; nothing in it asks the articles for a use they lack.
...
\mention{A} also determines quantificational determinatives. \textit{CGEL} lists \mention{a few}, \mention{a little}, and \mention{a great many} as complex determinatives while conceding that \mention{a few} \enquote{admits \mention{very}, \mention{good}, and \mention{fair}} and calling the internal structure \enquote{sui generis} \citep[392, 394]{huddleston2002}. Its own case against a complex quantifier \mention{a number of} is that a fixed phrase \enquote{doesn't provide a structure with potential for replacements and expansion}: the article is replaceable by other determiners and the noun takes adjectival modifiers \citep[351--352]{huddleston2002}. Both hold for \mention{few}: \mention{the few} and \mention{those few} beside \mention{a few} \citep[391, 393]{huddleston2002}, and \mention{a good few}, \mention{a fair few}, and \mention{a select few} beside \mention{a very few}.\footnote{\mention{A good few} and \mention{a fair few} are \textit{CGEL}'s \citep[392]{huddleston2002}. NOW: \enquote{I shouldn't have to leave for a select few who are uncomfortable} (\textit{New York Daily News}, 2022); \enquote{for those select few who know} (\textit{Electrek}, 2021).}

A Det NP with its own Det is required for \mention{the old man's hat}, so \mention{a few mistakes} has \mention{a few} in Det, an NP with \mention{a} as its Det and \mention{few} as its head, and \mention{a great many} is the same with an obligatory modifier \citep[394]{huddleston2002}. Definite \mention{the few mistakes} keeps \textit{CGEL}'s analysis, Det \mention{the} with modifier \mention{few} \citep[391]{huddleston2002}, since \mention{the} can determine \mention{mistakes} directly and \mention{a} can't. As a whole, \mention{a few} takes plural agreement in its fused-head use \citep[394]{huddleston2002}, the transparency of §\ref{sec:quant-nouns}.

The differences \textit{CGEL} records between \mention{a few} and \mention{several}, that \mention{quite}, \mention{not}, and \mention{only} combine readily with the first \citep[392]{huddleston2002}, are the combinations open to any indefinite-article NP: \mention{quite a mess}, \mention{not a soul}, \mention{only a handful}. Two facts stay lexical: \mention{several} takes definite determiners as a modifier \citep[392]{huddleston2002} but not \mention{a}, and the lower bound of three for \mention{a few} \citep[391]{huddleston2002} is a convention of the phrase, semantic motivation of the kind \textit{CGEL} itself declines to let establish a complex unit \citep[621]{huddleston2002}.

Degree \mention{such} and exclamative \mention{what} enter the same reanalysis from the adjective side. \textit{CGEL} classes them as adjectives because exclamative \mention{what} \enquote{fails all of the determinative criteria (a)--(c)}: it combines with \mention{a}, licenses no count singular head without it, and has no partitive use \citep[539--540]{huddleston2002}. The first two criteria test Det function, and in \mention{such a mess} and \mention{what a mess} the words aren't in Det.

\textit{CGEL}'s inventory of predeterminer modifiers includes \mention{all}, \mention{both}, fractions, multipliers, \enquote{a small range of adjectives (e.g.\ \mention{such} in [ic])}, and AdjPs such as \mention{how large a piece}, and \enquote{a predeterminer modifier normally precedes a determiner} \citep[331]{huddleston2002}. In \textit{CGEL} itself, then, \mention{such a mess} has \mention{a} in Det and \mention{such} as predeterminer modifier, and only the category of \mention{such} is at issue.

The same structure covers \mention{what a mess} and \mention{many a man}, which \textit{CGEL} treats as a \enquote{syntactically inert} complex determinative \citep[394]{huddleston2002}. Each inertness fact follows from the function. Nothing intervenes between the word and \mention{a} (\ungram{\mention{what long a speech}}) because a predeterminer sits against the determiner of its NP, as in \mention{all the books} and \mention{half a cake}, and the count singular head and singular agreement of \mention{many a man has} come from \mention{a} and \mention{man}.

Neither \mention{many a} nor \mention{what a} has an independent use, because a predeterminer isn't an NP, whereas \mention{a few} and \mention{a great many} are NPs with both determiner and fused-head use \citep[394]{huddleston2002}. That \mention{few} can't replace \mention{many} (\ungram{\mention{few a man}}) is lexical, as predeterminer membership is anyway (\ungram{\mention{most the books}}).

So \mention{many a} stands to \mention{many} as \mention{such a} to \mention{such} and \mention{what a} to \mention{what}. Each word has Det function with plural or non-count heads (\mention{many men}, \mention{such promise}, \mention{what fools}) and predeterminer function with an indefinite-article NP, where the modifier is an NP, as \textit{CGEL}'s fractions are \citep[385 n.~25]{huddleston2002}, or an AdjP (\mention{how large a piece}). No adjectival property remains: neither word has grade or degree modification, and exclamative \mention{what} has no predicative use.

\textcite[§5]{VanEynde_2007_BigMess} draws the same line from the other side: \mention{such a} and \mention{what a} aren't instances of the Big Mess construction (\mention{how long a bridge}), since \mention{what} and \mention{such} \enquote{are invariably lexical} and \enquote{lexically select a nominal which is either unmarked or introduced by the indefinite article}. I treat degree \mention{such} and exclamative \mention{what} as determinatives with these two functions; the kind sense of \mention{such} (\mention{such contract as may be executed}, \mention{many such problems}) is the determinative \mention{such} of \textit{CGEL}'s inventory. With \mention{a few} an externally determined NP and \mention{many a} a predeterminer construction, no complex determinatives remain in this account; \textit{CGEL}'s other members of that class, cardinals above a hundred, are the subject of \textcite{reynolds2026numerals}.
...
The argument is synchronic: the development of definite articles from demonstratives and of articles expressing cardinality from numerals \citep[331--336]{lyons1999} bears on content and use, not on the categorization.
...
The D-noun analysis pairs the proposed categorization with ordinary Head. This section compares that treatment with fusion in independent \mention{some}, \mention{some of the wine}, and \mention{the lucky few}. Ordinary headedness gives the determinative Head alone; fusion combines Head with a dependent function.
...
The comparison with ordinary pronouns and proper nouns is structural as well as interpretive. In \mention{She left} and \mention{Kim left}, \textit{CGEL} permits an NP headed by a nominal with no determiner. Applying that structure to \mention{Some left} preserves the same division between a complete NP and its context-dependent reference.

Fusion represents the same complete expression differently: in \textcite{Payne2007}, one constituent jointly realizes Head and a dependent function, subject to structural and adjacency conditions. Both analyses keep an overt determinative and can express the restriction supplied by context; §\ref{sec:costs} compares them on the structures they share.
...
Figure~\ref{fig:few} compares the resulting analyses of \mention{the lucky few}. Both have an overt Det and a Nom modified by \mention{lucky}. In the D-noun analysis, the lexical word \mention{few} directly heads Nom. In the fusion analysis, its DP fills Mod--Head, the machinery of adjectival fusion.
...
Reducing fusion's applications can simplify this description even when fusion remains available for adjectives. The advantage is the uniform treatment of \mention{few}; its value depends on any additional conditions or lost generalizations elsewhere. Section~\ref{sec:costs} compares those costs while holding the lexical restrictions fixed.

\mention{The remaining three} extends the modifier pattern to cardinals. It doesn't independently decide whether \mention{three} is determinative or has a common-noun use: \textcite{reynolds2026numerals} argues for both uses of cardinals.
...
In \mention{some apples} and \mention{Kim's apples}, the determining phrases share the NP category under D-noun with ordinary Head. For D-noun with fusion, I retain that NP category and add the combined function relations in independent uses. \textit{CGEL} instead distinguishes DP from genitive NP. It also admits restricted plain-case NPs and PPs as determiners, as in \mention{what size hat} and \mention{over thirty ties} \citep[Ch.~5, §4]{huddleston2002}.

I carry this PP-determiner provision over to the three NP-projecting implementations for this comparison. Its inclusion in the alternatives is an extrapolation.
...
Every account states this condition for common nouns heading Nom. Fusion is held fixed for adjectives in all four implementations, so what the two ordinary-Head implementations differ in is how they remove the determinative fusion: D-noun moves the determinatives one node down the taxonomy and keeps the condition as it is, while separate D keeps D a primary category and adds the disjunct. The coverage audit marks the derivations that go through it (§\ref{sec:overall-assessment}). Both permit the same nominal layers, selected complements, and ordered modifiers.
...
\mention{A few} and \mention{a little} are NPs in Det with \mention{a} as their own Det and \mention{few} or \mention{little} as head (§\ref{sec:article-membership}); the internal degree modifier of \mention{a very few} attaches to that head and peripheral \mention{quite} to the NP. \mention{Many a} is \mention{many} in predeterminer function on an \mention{a}-NP, which keeps it to dependent use (§\ref{sec:article-membership}).

The constraints also distinguish the uses of \mention{few}. In \mention{the few people}, its phrase is an internal Mod of the nominal headed by \mention{people}; in \mention{a few people}, \mention{few} heads the NP in Det; in \mention{the lucky few}, \mention{few} heads the independently determined NP.
...
\mention{Enough} also modifies adjectives, adverbs, verbs, and some PPs: \mention{good enough}, \mention{quickly enough}, \mention{I hadn't prepared enough}, and \mention{enough in control} \citep[396--397]{huddleston2002}. The degree determinatives \mention{much} and \mention{little}, and \mention{no}/\mention{none}, likewise have uses outside NP structure \citep[390, 395--397]{huddleston2002}. \textcite{reynolds2024why} surveys the determinatives that modify AdjPs, AdvPs, and PPs (\mention{more}, \mention{less}, \mention{enough}, \mention{that}, \mention{much}, \mention{a little}, \mention{all}) and finds their distribution idiosyncratic lexeme by lexeme, as adjective intensifiers are generally, with no pattern that sets any of them apart as an adverb; the modifying phrases take modifiers of their own (\mention{far fewer}, \mention{much more}, \mention{so much}), the internal structure an NP headed by a determinative supplies.\footnote{\mention{Both}, \mention{either}, and \mention{neither} also serve as markers of coordination \citep[1305, 1308]{huddleston2002}. Their noun categorization would preserve that further category--function combination; this fragment doesn't analyse coordination.}

NPs modify adjectives in \mention{three years old}, \mention{a great deal smaller}, and \mention{plenty big enough}. \textit{CGEL} also treats \mention{lots better} and \mention{heaps worse} as containing quantificational NP modifiers \citep[549--550]{huddleston2002}. These uses extend the connection with the quantificational common nouns compared in §\ref{sec:quant-nouns}.
...
Separate D with ordinary Head licenses the same structures by admitting N or D. The advantage claimed for D-noun is the correspondence between the lexical grouping and this shared grammar, and the coverage audit (fifth supplement) says how much that correspondence saves. For each construction family of the fragment and each lexeme group, it records how each implementation covers the facts: by a general rule, by a rule stated once for nouns and again for determinatives, by a fact on the lexeme, or by a condition for that construction alone. Of 103 cells, 35 are lexical facts identical in all four accounts. The fusion accounts state where partitives and relatives attach, outside the fused phrase, as conditions on those constructions, and \textit{CGEL}'s grammar states the peripheral-modifier, number-transparency, and degree-modifier rules once for NPs and again for DPs. Separate D with ordinary Head avoids both as well, and the audit marks how: 32 of its derivations go through the disjunct that lets a determinative head Nom and NP. The two ordinary-Head implementations share the saving and buy it differently, D-noun with a reclassification, separate D with a condition added and a primary category kept. For the same coverage the separate category therefore carries strictly more: the disjunct; a category whose defining profile duplicates Noun's, since the partitives, number transparency, restricted dependents, genitive marking, number, and reference that would characterize it are the nominal connections of §\ref{sec:proposal}, and the dependent and independent form pairs it needs are the ones pronouns already have within Noun; and the conversion rule and the extended genitive definition that the secondary use and \mention{everyone's} require of it. What the extra machinery buys is the boundary itself, and the profile comparison is where the boundary is decided. A feature can't do that work more cheaply: a nominal feature carried by exactly the nouns and the determinatives is the superordinate category under another name, and a feature earns its place only by cross-cutting the categories, which in this fragment none does. A recoding that prices the disjunct at zero brings the two level (fifth supplement); that price is the assumption on which a defence of the separate category rests. The secondary use and the former complex determinatives weren't among the diagnostics of §\ref{sec:proposal}; they're additional consequences, not tests set in advance.

The grouping has a further consequence. Compound determinatives and proper nouns share a secondary set-denoting use in which the word takes an article, a pre-head adjective, and plural inflection (§\ref{sec:compounds}): \mention{a special someone} and \mention{such an anyone} beside \mention{an Ophelia} and \mention{the Smiths}. \textit{CGEL} describes the proper-noun case as a secondary use within Noun and must treat the compound case as conversion to another category \citep[423, 521]{huddleston2002}. Within Noun, one secondary use covers both, and the article and the adjective follow from what the word now denotes. A separate primary D needs a conversion rule to reach the same facts; the subcategory analysis unifies them. \textit{CGEL} makes the same move a third time for cardinal numerals, which are \enquote{primarily determinatives but they have a secondary use in which they inflect for number and hence belong in the noun category}: \mention{in threes}, \mention{in their hundreds} \citep[385]{huddleston2002}. Within Noun the plural cardinal is the same secondary use again, with no category to change; \textcite{reynolds2026numerals} supplies the separate case that cardinals have both uses.

The same pattern recurs where this paper has so far spoken of connections. The transparency rule for partitives is stated once for \mention{lot}, \mention{number}, and \mention{some} (§\ref{sec:quant-nouns}); one condition covers NP and determinative degree modifiers of adjectives (§\ref{sec:enough}); one peripheral attachment covers \mention{almost every} and \mention{only you} (§\ref{sec:payne}); and one definition of the head genitive covers \mention{everyone's} and \mention{Edward's} (§\ref{sec:inflection}). Each is a rule \textit{CGEL} states separately for nouns and for determinatives, and its complex determinatives dissolve into two of the account's existing constructions, external determination for \mention{a few} and predeterminer modification for \mention{many a} (§\ref{sec:article-membership}). Each of these is a saving of ordinary Head with NP projection, available to a separate category at the price just stated. So the economy the analysis claims is the economy of removing one use of fusion by reclassification rather than by a disjunct and a duplicated category; the profile of §\ref{sec:proposal} is the prior case for the reclassification, and the audit shows what it then buys in the grammar.
...
The ordinary-Head analysis uses the same chain in bare, partitive, and externally determined expressions: the word heads Nom, and Nom heads NP. The taxonomic and structural choices complement each other: the nominal profile supports grouping determinatives with nouns, and that grouping satisfies the proposed grammar's category condition on lexical Heads in Nom. The coverage audit records the price of the alternative: a separate category reaches the same coverage only with a disjunct in its category condition, a category whose profile duplicates Noun's, and a conversion rule and an extended genitive definition. The grouping also unifies three secondary uses that \textit{CGEL} keeps apart: the set-denoting compound (\mention{a special someone}), the set-denoting proper noun (\mention{an Ophelia}), and the plural cardinal (\mention{in threes}) fall under one process once all three are nouns (§\ref{sec:compounds}, §\ref{sec:overall-assessment}), and the complex determinatives \mention{a few} and \mention{many a} dissolve into external determination and predeterminer modification (§\ref{sec:article-membership}).
...
Van Eynde (\citeyear{vaneynde2003determiner}) & Determiners categorially heterogeneous on inflection and agreement: those that inflect like prenominal adjectives are adjectives; genitives and non-agreeing pronouns are nouns (Italian, Dutch; see §\ref{sec:four-questions}). & Lexical taxonomy, cross-linguistic \\
