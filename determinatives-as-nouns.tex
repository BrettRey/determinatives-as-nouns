...
The condition is substantive. Every account states it for common nouns heading Nom; D-noun extends its domain to a fourth subcategory, and separate D with ordinary Head admits a second category into it, the admission the coverage audit marks (§\ref{sec:overall-assessment}). Under D-noun, determinatives satisfy the condition as members of Noun; separate D admits them alongside Noun. Both permit the same nominal layers, selected complements, and ordered modifiers.
...
Separate D with ordinary Head licenses the same structures by admitting N or D. The advantage claimed for D-noun is the correspondence between the lexical grouping and this shared grammar, and the coverage audit (fifth supplement) says how much that correspondence saves. For each construction family of the fragment and each lexeme group, it records how each implementation covers the facts: by a general rule, by a rule stated once for nouns and again for determinatives, by a fact on the lexeme, or by a condition for that construction alone. Of 103 cells, 36 are lexical facts identical in all four accounts. The fusion accounts state where partitives and relatives attach, outside the fused phrase, as conditions on those constructions, and \textit{CGEL}'s grammar states the peripheral-modifier, number-transparency, and degree-modifier rules once for NPs and again for DPs. Separate D with ordinary Head avoids both, and the audit marks how: 32 of its derivations go through the admission that a determinative heads Nom and NP, the structure that defines Noun in \textit{CGEL}'s own terms. That admission is the D-noun categorization stated as a disjunction, with a separate category left to do no work in the syntax. Three cells then separate Noun membership from a separate D even with the admission made: the secondary set-denoting use of compounds and of cardinals, which a separate D covers by a conversion rule, and the head-genitive definition, which a separate D has to extend to its own heads. Neither the secondary use nor the former complex determinatives was among the diagnostics of §\ref{sec:proposal}, so the grouping covers them without having been built from them.
