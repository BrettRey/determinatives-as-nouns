...
Table~\ref{tab:existing} summarizes the range within each category; individual lexemes differ in which properties they exhibit. Functions concern phrases, inflection concerns word forms, and inventory closure concerns categories. AdjP and AdvP denote adjective and adverb phrases. The modifiers row records which modifiers occur; where they attach is the analysis of §\ref{sec:payne}. The rows aren't equally weighted tests, and several properties belong to one connected pattern.
...
Under D-noun, determinatives satisfy the first condition as members of Noun. Separate D admits them alongside Noun. Both permit the same nominal layers, selected complements, and ordered modifiers.
