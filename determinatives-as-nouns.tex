...
The complex-determinative precedent extends to degree \mention{such} and exclamative \mention{what}, which \textit{CGEL} classes as adjectives because exclamative \mention{what} \enquote{fails all of the determinative criteria (a)--(c)}: it combines with \mention{a}, licenses no count singular head without it, and has no partitive use \citep[539--540]{huddleston2002}. Read as \textit{CGEL} reads \mention{a few} and \mention{many a}, the first two criteria are met by the complex forms \mention{such a} and \mention{what a}, which are as inert as \mention{many a} (\ungram{\mention{what long a speech}}) and always function as Det of a count singular NP; the partitive criterion fails for the articles as well. In non-count and plural NPs (\mention{such promise}, \mention{what fools}) the word is NP-initial with no other determiner, where \mention{much} and \mention{many} have Det. No adjectival property remains: neither word has grade or degree modification, and exclamative \mention{what} has no predicative use. \textcite[§5]{VanEynde_2007_BigMess} reaches the same division from the other side: \mention{such a} and \mention{what a} aren't instances of the Big Mess construction (\mention{how long a bridge}), since \mention{what} and \mention{such} \enquote{are invariably lexical} and \enquote{lexically select a nominal which is either unmarked or introduced by the indefinite article}. I treat degree \mention{such} and exclamative \mention{what} as restricted determinatives, dependent-only like the articles, with \mention{such a} and \mention{what a} as complex determinatives beside \mention{many a}; the kind sense of \mention{such} (\mention{such contract as may be executed}, \mention{many such problems}) is the determinative \mention{such} already in the inventory.
