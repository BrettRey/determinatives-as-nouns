...
Revised after your final structural comments. Independent determinatives consistently receive ordinary Head, with modifier attachment specified by construction; anaphoric genitive fusion is retained explicitly. Trees and schemata are shown as text; footnotes appear beside their paragraphs. [Typeset PDF](../determinatives-as-nouns.pdf) · [Response to all review points](passes/2026-09-12-head-consistency.md).
...
| Modification | Productive AdjP and nominal premodifiers; relative postmodifiers | Restricted AdjP premodifiers and embellishments | Restricted AdjP premodifiers and relative postmodifiers | AdvP and D premodifiers; nominal postmodifiers; *the lucky few* |
...
Shared nominal projection doesn’t make modifier permissions uniform. In *almost every experienced teacher*, *almost* belongs with *every*, while *experienced* modifies *teacher*. Figure 3 gives *almost* peripheral attachment within the smaller determinative-headed NP. That NP functions as Det in the larger NP, whose ultimate head is *teacher*.
...
*CGEL* already observes overlap between DP modifiers and peripheral NP modifiers, with some attachment decisions uncertain (Huddleston and Pullum 2002, 431). The D-noun analysis assigns peripheral attachment to adverbial premodifiers of determinative-headed NPs without external determination: *almost every*, *hardly any*, *hardly anyone present*, and *almost ten*.

NP embedding preserves this distinction. In *the very few people*, I analyse *very few* as an NP functioning as Mod, with *very* peripheral to the NP headed by *few*. The determiner *the* belongs to the larger NP; the smaller NP has no external Det.

Independent *the very few who objected* instead has *few* as ordinary Head and *very* as an internal modifier of Nom, like *lucky* in *the lucky few*. The same allocation applies to *the very many* and *the almost thirty who came*. In these ordinary-Head structures, the adverb follows the NP’s own external Det and occupies its internal premodifier position.

NP modifiers provide a further connection. Determinatives permit determinative premodifiers such as *this* in *this much* (Huddleston and Pullum 2002, 393). Comparative determinatives also take established NP modifiers: *a lot* in *a lot fewer*, alongside *CGEL*’s *a lot more than fifty* (Huddleston and Pullum 2002, 432). Under the proposed analysis, these are peripheral NP modifiers of a determinative-headed NP without external Det. Their permissions remain specific to the head and construction.
...
This distinction shows why nounhood and fusion are compatible. Ordinary headedness follows here from the proposed projection rules; it doesn’t require eliminating every fused construction containing a noun. I retain *CGEL*’s fusion analysis for anaphoric independent genitives such as *mine*, while assigning ordinary Head to independent determinatives such as *few*.
...
Both ordinary-Head accounts use peripheral attachment without external determination and internal AdvP attachment with externally determined gradable quantifiers and cardinals (§3.4). D-noun states that internal permission within Noun. Generalizing NP degree modification carries a further cost for both accounts, extending the already qualified restriction with its *a lot* exception (§6.2).
