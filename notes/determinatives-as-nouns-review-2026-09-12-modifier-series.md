...
Revised to distinguish peripheral approximative and focusing modifiers from internal degree modifiers. The a very few / quite a few contrast is checked against CGEL p. 392; the profile and comparative accounting reflect the distinction. Trees and schemata are shown as text; footnotes appear beside their paragraphs. [Typeset PDF](../determinatives-as-nouns.pdf) · [Response to all review points](passes/2026-09-12-modifier-series.md).

# Abstract

I argue that English determinatives belong within Noun alongside common nouns, proper nouns, and pronouns. Their combined semantic, syntactic, and morphological profile supports the grouping; quantificational common nouns provide particularly close connections. Grade and its associated degree-modifier pattern distinguish four quantifiers; peripheral modification follows existing NP grammar. Lexical and constructional permissions within the subcategory accommodate restricted articles and degree-modifier uses. The proposed hierarchy makes nominal projection inherited, unifying bare, partitive, and externally determined uses under ordinary Head structure. A matched grammatical fragment compares separate-D alternatives and identifies the costs of restricting modifier generalizations to subcategories.
...
Table 2: Representative profiles. Functions concern phrases; internal and peripheral modification concern Nom and NP respectively. Interrogative and relative forms follow *CGEL*’s inventory.
...
> Note 5. Whether independent *what* and relative *which* belong to pronoun or determinative leaves them within the proposed Noun category. I leave that internal boundary open here; *CGEL*’s inventory suffices for the present comparison (Huddleston and Pullum 2002, 397–99).
...
Modifier and adjunct functions also cut across the groups. Compare the modifiers in *dog houses*, *Canada Day*, *the manager herself*, and *the few people*. Emphatic *herself* also functions as a clause adjunct in *The manager detected the error herself* (Huddleston and Pullum 2002, 1496–97). Temporal NPs such as *that day* and *Sunday* supply adjuncts, as does degree *enough* in *I hadn’t prepared enough* (§6.2). These are shared functions with construction-specific distributions.
...
The strongest adjectival connection is concentrated in *few*, *many*, *much*, and *little*. They inflect for grade, take degree modifiers such as *very* and *so*, and license comparative *than*-complements. These properties form a connected profile. Predicative use extends to *several*, but the broader adverbial pattern involves approximative and focusing modifiers, analysed below at the NP periphery. Even the gradable forms have distinct syntax: *so many mistakes* contrasts with \**so numerous mistakes* (Huddleston and Pullum 2002, 392–96, 431–32, 539–40).

The case for inclusion rests on this distribution of connections. Determinatives share argument and determiner functions with all three noun subcategories, referential contrasts with pronouns, and number, genitive marking, and partitive constructions with established parts of Noun. Approximative and focusing modification reuses independently established NP grammar (§3.4). The localized degree-modifier, grade, and complementation pattern supplies an adjectival counterweight. I give greater weight to the recurring nominal profile, while preserving that connected set of distinctive properties within the determinative subcategory.
...
## 3.2 The connection from the common-noun side

Quantificational common nouns make the connection more specific than shared quantity meanings. In *a lot of the delegates* and *many of the delegates*, both heads quantify over a partitive domain. But *lot* also permits *a lot of delegates*, whereas \**many of delegates* is excluded (Huddleston and Pullum 2002, 349). The comparison links the groups through a shared construction while preserving a specific difference in complementation.
...
## 3.4 Modifier selection and taxonomic level

Two premodifier patterns need distinguishing: approximative and focusing adverbs, and the degree series exemplified by *very* and *so*. In *almost every experienced teacher*, *almost* belongs with *every*, while *experienced* modifies *teacher*. Figure 3 places *almost* at the periphery of the NP headed by *every*. That smaller NP functions as Det in the larger NP, whose ultimate head is *teacher*.
...
Figure 3: Peripheral *almost* modifies the smaller NP headed by *every*; the resulting *almost every* NP functions as Det. The adjective *experienced* modifies the common-noun nominal. Internal structure within the one-word modifier phrases is suppressed.
...
This pattern extends broadly across determinatives: *almost every*, *nearly all*, *hardly any*, *practically no*, *almost everyone*, and *almost ten*. Under D-noun, these combinations reuse the peripheral modification found in *almost the whole class*, *hardly a day*, and *only the best*. In dependent *almost every student*, *almost* approximates the quantificational contribution of *every*; it doesn’t intensify a gradable property of that head. The NP analysis gives this broad pattern a place in existing nominal grammar.

Degree *very* has different selection: *very tall* and *very few*, but \**very you*, \**very the book*, and \**very my life*. It also excludes non-gradable determinatives: \**very every*, \**very some*, and \**very this*. The associated series includes *so few*, *too few*, *how few*, and *as few as*. These degree patterns select *few*, *many*, *much*, and *little*, the four determinatives that inflect for grade (Huddleston and Pullum 2002, 393–95, 431–32).

> Note 7. The restriction concerns this degree series. *CGEL* also records modifiers of completeness, including *marginally enough* and *absolutely all*, with different selectional ranges (Huddleston and Pullum 2002, 432). Those combinations don’t extend the *very*/*so*/*too*/*how* series beyond the four degree determinatives.

The complex determinative *a few* makes the attachment contrast visible. *CGEL* explicitly contrasts internal *very* in *a very few mistakes* with peripheral *quite* in *quite a few mistakes* (Huddleston and Pullum 2002, 392, (61)). The reversed orders \**very a few* and \**a quite few* are excluded in these constructions. Here *a* belongs to the complex determinative; *very* occurs within it and *quite* outside it.
...
The *no*/*none* alternation distinguishes dependent and independent forms. Genitive *my*/*mine* supplies a parallel (§6.2). Inflection thus supports both nominal and adjectival connections. The clearest nominal comparison is demonstrative number; grade is the clearest counterweight. Neither can serve as a necessary condition for the whole superordinate category.

Derivation supplies a weaker category diagnostic. Determinatives enter common-noun formations such as *nothingness* and *oneness*, but *-ness* also attaches to adjectives and other bases (Huddleston and Pullum 2002, Ch. 19, §5.7.2(i)). Such outputs don’t establish a uniformly noun-selecting rule. The inflectional comparisons above bear more directly on the proposed grouping.

> Note 8. Numeral morphology also needs the input category distinguished from the output. Reynolds (2026, secs. 5.4–5.5) derives fractional nouns from cardinal nouns and treats *two thousand and twenty-seventh* as a coordination whose final coordinate is adjectival. Neither process independently establishes determinative nounhood.
...
> Note 10. CGELBank (Reynolds et al. 2023), sentence [`reviews-083459-0002`](https://github.com/nert-nlp/cgel/blob/d0a2c2d8c522b23d5aa879fc9cb94948f4285b2c/datasets/ewt.cgel#L651-L654).
...
The proposed analysis gives *mine* ordinary Head in both anaphoric and predicative uses. Its genitive form expresses a relation to the speaker; context can supply the understood nominal description, as it can with independent *few*. The *my*/*mine* alternation remains a form-selection condition, parallel to *no*/*none*.
...
In the ordinary-Head tree, *anyone* inherits nominal projection from Noun and its premodifier permissions from its determinative base. Compounds exclude external determination, so *hardly* occupies the NP periphery. The compound construction supplies the internal post-head restrictor permission, with the same position and non-recursion conditions as the fusion account. It doesn’t license a corresponding pre-head adjective.
...
> Note 13. Sentence [`answers-20111024111513AAAQhAO_ans-0003`](https://github.com/nert-nlp/cgel/blob/d0a2c2d8c522b23d5aa879fc9cb94948f4285b2c/datasets/ewt-test_iaa50.cgel#L179-L181) in `ewt-test_iaa50.cgel`.
...
# 6 Restricted members and wider functions

The broader grouping includes members with restricted distributions. Articles test whether ordinary independent use is necessary; *no*/*none* tests form selection, and *enough* extends the comparison to functions outside NP structure.
...
## 6.2 Form selection and degree modification
...
The D-noun analysis applies nominal projection to determinatives through their membership in Noun. A separate condition checks use permissions. In the schemata below, `h` identifies the lexical head throughout its projection; parentheses mark optional dependents. The head’s entry and construction restrict every dependent. `Mod_periph` adds the peripheral premodifier discussed in §3.4; the compact rule leaves further NP layering implicit.
...
The disjunction `{N, D}` groups the same heads for nominal projection. Expressing that grouping in the lexical hierarchy gives the D-noun analysis a point of harmony between taxonomy and phrase structure. This earns no extra credit for shared rules: both ordinary-Head accounts state them. The preference for including D within Noun rests on the broader profile, as the comparison below makes explicit.
...
Table 5 compares five criteria across the three accounts, holding the inventory, constructions, and readings fixed. Existing restrictions are distinguished from changes required by the chosen projection rules. Shared rules receive equal credit whether stated through inheritance or a disjunction of categories.
...
| Internal differences | Internal degree AdvP on *few*, *many*, *much*, *little*; approximatives after Det with independent cardinals. Compound restrictor order and non-recursion retained. | Same internal permissions, stated for primary D; same peripheral NP modification and compound conditions. | DP and Nom separate modifier domains. Same compound restrictor order and non-recursion. |
| Related forms and uses | Cardinal determinatives and cardinal nouns fall within Noun, as do the three lexemes spelled *one*. Internal distinctions retained. | Determinative cardinals and determinative *one* remain primarily separate from nominal counterparts. | Same primary-category divisions as separate D with ordinary Head. |
| Restrictions required | Article-use permissions and *no*/*none* selection retained. Degree uses extend the already qualified restriction on NP modifiers in attributive AdjPs. | Same retained permissions and form selection. Same degree-related extension when NP projection includes these uses. | Same article permissions and form selection. DP–NP distinction retains the degree contrast, including exceptional NP constructions. |
...
The ordinary-Head accounts share peripheral NP rules for approximative and focusing modifiers. Their internal permissions cover the degree series on the four gradable quantifiers and approximatives on externally determined independent cardinals (§3.4). D-noun states these permissions within Noun. The localized degree series accompanies grade and comparative complementation; the broader adverbial pattern fits NP structure. Generalizing NP degree modification outside nominal constructions carries a further cost for both accounts, extending the already qualified attributive-AdjP restriction with its *a lot* exception (§6.2).

On shared structure, the ordinary-Head accounts tie. D-noun reduces the number of primary categories by one, retaining determinative as a subcategory. The preference for that grouping depends on the semantic, syntactic, and morphological comparison, especially the common-noun connections in §3.2.
...
These cardinals and the lexemes spelled *one* retain their distinctions within one primary category. Their grouping follows from the proposed hierarchy, whose support comes from the broader profile comparison.
...
The resulting category retains internal differences. Articles lack ordinary independent uses; the *no*/*none* paradigm requires form selection; degree modifiers require construction-specific permissions. These distinctions constrain the proposed subcategories within their shared parent.
