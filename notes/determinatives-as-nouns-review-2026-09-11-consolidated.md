...
Revised after your correction and the two supplied assessments. The pronoun-boundary section is removed, embedded NP modification is distinguished from outer NP structure, and the comparison of grammatical commitments is consolidated. Trees and schemata are shown as text; footnotes appear beside their paragraphs. [Typeset PDF](../determinatives-as-nouns.pdf) · [Response to all review points](passes/2026-09-11-consolidation.md).
...
These levels remain distinct when one NP is embedded inside another. In *the very few people*, *very few* can be an NP functioning as Mod, with *very* peripheral to the NP headed by *few*. The determiner *the* belongs to the larger NP. Its position before *very* doesn’t make *very* internal to the smaller NP.

The same attachment is available in independent *the very few who objected*, with the inner NP realizing Mod–Head fusion. Alternatively, an ordinary-Head analysis can place *very* within the nominal headed by *few*. Nounhood permits both options: it doesn’t itself eliminate fusion (§4.2). The example therefore doesn’t establish that internal AdvP modification is unavoidable.

*CGEL* already observes overlap between DP modifiers and peripheral NP modifiers, with some attachment decisions uncertain (Huddleston and Pullum 2002, 431). Replacing DP with NP makes peripheral attachment available within determinative-headed projections. Where an ordinary-Head account instead chooses internal attachment, it needs the corresponding modifier permission. That is a property of the chosen structure, rather than a cost forced by the outer determiner.
...
Fusion reuses the DP–Nom boundary to separate the two modifier domains. The displayed ordinary-Head tree locates both within Nom, assigning their restrictions to the determinative base and compound construction. Peripheral NP attachment for *hardly* is another possibility (§3.4); either structure retains the compound’s post-head restrictor condition.
...
*Some* and *few* permit a partitive *of*-phrase within their nominal projection. Peripheral *almost* can modify the NP headed by *every*; this doesn’t license *experienced* as its modifier. The construction *the lucky few* permits the external determiner and adjectival modifier shown in Figure 5.
...
Table 5: Comparative accounting. These are descriptive commitments, not numerical scores. The degree-use comparison extends the ordinary-Head accounts beyond the fragment’s NP-internal constructions.
...
Modifier structure offers choices rather than a single taxonomic test. Peripheral attachment can reuse NP grammar, including inside a smaller NP in Mod function (§3.4). Where the ordinary-Head description uses internal AdvP attachment, it needs corresponding permissions within Nom. The cost of generalizing NP degree modification is firmer: it applies to both ordinary-Head accounts and extends an existing restriction with an existing *a lot* exception (§6.2).
