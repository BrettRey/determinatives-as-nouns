...
Why this shape: the exclusion is kept for the determinative and the counterexamples are given their analysis, with the corpus supplying the productive cases (*anonymous*, *responsible*, *furry*) that make this a rule rather than a list of two idioms.

**Revision to make before applying (2026-09-17, after discussion):** drop "by conversion". Under the D-noun analysis this is the same secondary set-denoting use that *CGEL* gives proper nouns in *an Ophelia*, *the two Marys*, *the Smiths* (p. 521: the name "denotes the set of people bearing this name" and "as a set-denoting noun … takes a full range of dependents, including determiners and restrictive modifiers"), not a category change; n. 43 stays as *CGEL*'s acknowledgement of the data. The shift is productive across the construction, *any-* compounds included (*such an anyone*, *Bones*, 2010; *I qualified as an anybody*, *Skiing*, 1992; *an anywhere operation*, web), so do not claim a lexical restriction. Diagnostics to state: the use denotes a kind or an individual of the quantifier's description, quantificational force is lost, plural becomes available (*somebodies*, *the little somethings*), and the quantifier and its nominal echo can sit in adjacent sentences. The manuscript already relies on the proper-noun half at line 184 (*the Smiths*); one account now covers two subcategories where *CGEL* needs conversion for one and a secondary use for the other, which is a point for the taxonomy. The *Analog* line is the best single citation because the next sentence takes *the something*. The second footnote is honest about the one thing the corpus can't show.
