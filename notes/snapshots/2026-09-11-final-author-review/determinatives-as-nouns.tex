...
\begin{abstract}
I argue that English determinatives belong within Noun alongside common nouns, proper nouns, and pronouns. Their combined semantic, syntactic, and morphological profile supports the grouping; quantificational common nouns provide particularly close connections. Adverbial premodification and grade distinguish determinatives within it. I examine restricted articles, reconsider independent \mention{what}'s pronoun classification, and address determinatives' degree-modifier uses. The proposed hierarchy makes nominal projection inherited, unifying bare, partitive, and externally determined uses under ordinary Head structure. A matched grammatical fragment compares separate-D alternatives and identifies the costs of restricting modifier generalizations to subcategories.
\end{abstract}
...
\textcite[153--158]{spinillo2004reconceptualising} instead retains \mention{the}, \mention{a}, and \mention{every} as an expanded article category and redistributes other determinatives. Her grounds include dependence on a following nominal, lack of predicative and partitive uses, and limited descriptive content. She also recognizes differences within the trio. Section~\ref{sec:articles} assesses this cluster against the connections each member retains with the wider inventory.
...
Meaning and reference vary within Noun. \mention{Apple} describes a class; \mention{Kim}, in its primary naming use, identifies through a name; \mention{she} depends on context. Determinatives range from deictic \mention{this} to quantitative \mention{many} and universal \mention{every} \citep[358--360, 370--405, 425--428, 515--521]{huddleston2002}. Deixis connects demonstratives with pronouns, while quantity connects determinatives with common nouns such as \mention{number} and \mention{majority}.
...
Interrogative, relative, and exclamative properties must be distinguished from those functions. An interrogative object may be headed by a pronoun (\mention{who}) or a common noun (\mention{which book}); the latter obtains its interrogative property from a dependent. Similarly, \mention{What a nice day!} requires exclamative \mention{what}; substituting an ordinary NP doesn't preserve the construction. \textit{CGEL} classifies this \mention{what} as an adjective, distinct from interrogative and relative \mention{what} \citep[433--437; Ch.~10, §7]{huddleston2002}.
...
Determinatives also permit determinative premodifiers, as in \mention{this much} \citep[393]{huddleston2002}. Under the proposed analysis, this is a noun modifying another noun. Its licensing remains specific: the availability of one determinative modifier doesn't license arbitrary nominal premodification.
...
Derivation supplies a less uniform comparison than inflection. Noun-forming suffixes differ in the bases they select: \mention{-ship} in \mention{friendship} primarily selects nouns denoting persons, though \mention{hardship} has an adjective base; the inhabitant suffix \mention{-er} in \mention{Londoner} attaches to a place name \citep[Ch.~19, §§5.6.2(b), 5.7.2(k)]{huddleston2002}. No noun-selecting suffix shared by pronouns and determinatives is established here.
...
The pronoun inventory in that study follows \textit{CGEL}'s classification, whereas Hudson's pronoun category includes the words he calls determiners. The reassignment of independent \mention{what} and relative \mention{which} proposed below also changes the boundary. The archived analysis therefore isn't a test of the revised membership proposed here. Comparing the hierarchies requires aligned lexical inventories and generalizations stated for each candidate superordinate category.
...
\mention{Who} has positive morphological grounds for pronoun status: its paradigm includes nominative \mention{who}, accusative \mention{whom}, and genitive \mention{whose} \citep[904--905]{huddleston2002}. \mention{What} lacks a comparable case paradigm. That absence alone wouldn't exclude it from pronoun, but independent use and a non-personal interpretation don't establish pronounhood either. Both properties occur among determinatives, including the \mention{-thing} compounds.

\textit{CGEL} assigns interrogative \mention{what} to two categories. Before an overt nominal it's determinative, as in \mention{what book} or \mention{what person}; independently it's a non-personal pronoun, as in \mention{What arrived?} Relative \mention{what}/\mention{whatever} receives a corresponding split. The stated contrast concerns gender: the independent forms are non-personal, while the determinatives permit personal and non-personal heads \citep[398]{huddleston2002}.

I treat these occurrences as determinatives throughout. With an overt nominal, that nominal supplies the restriction; without one, the construction requires a non-personal interpretation. This states the contrast as a use condition instead of assigning a second lexical category on its basis. The determinative analysis preserves \mention{what}'s connection across the constructions while distinguishing its gender restrictions from those of \mention{which}.

Interrogative \mention{which} is already determinative in \textit{CGEL} both before a nominal and independently: compare \mention{Which candidate arrived?} and \mention{Which arrived?} With candidates under discussion, the latter can be personal. It also permits \mention{which of the candidates}, whereas \ungram{\mention{what of the candidates}} doesn't have the corresponding selective reading \citep[397--398]{huddleston2002}. Determinative membership therefore doesn't predict identical gender or partitive permissions.

Relative \mention{which} poses the same boundary question as \mention{what}. \textit{CGEL} treats it as determinative in \mention{at which stage}, but as a non-personal pronoun in \mention{the post office, which is nearby} \citep[399]{huddleston2002}. I retain determinative here too, with the non-personal restriction attached to this relative construction. This changes lexical classification, leaving relative-clause syntax, including fused relatives, to be analysed separately. Either classification places these words within the proposed superordinate Noun.

Exclamative \mention{what} in \mention{What a nice day!} remains a separate case. Following \textit{CGEL}, I retain its adjective analysis here (§\ref{sec:membership}). A common spelling doesn't require one lexeme across all constructions. Nor does being a \term{pro-form} determine lexical category: gender-sensitive interpretation must be distinguished from the grammatical evidence for pronoun membership \citep{reynolds2025proformgender}.
...
The resulting category retains internal differences. Articles lack ordinary independent uses; the \mention{no}/\allowbreak\mention{none} paradigm requires form selection; degree modifiers require construction-specific permissions. I retain \mention{who} as a pronoun and analyse interrogative and relative \mention{what} as determinative, with explicit interpretive restrictions. These distinctions constrain the proposed subcategories within their shared parent.
