...
root = Path.cwd()
snap = root / 'notes/snapshots/2026-09-12-bounded-repairs'
before = (snap / 'determinatives-as-nouns.tex').read_text()
after = (root / 'determinatives-as-nouns.tex').read_text()
review = root / 'notes/determinatives-as-nouns-review-2026-09-12-bounded-repairs.md'
md = review.read_text()
...
checks['schemata'] = {'count': 4, 'change': 'All four schemata unchanged'}
...
checks['numbered_examples'] = {'count': 2, 'words_unchanged': True, 'format_change': 'None'}
...
assert [i for i,(a,b) in enumerate(zip(old_tables,new_tables)) if a!=b] == []
checks['tables'] = {'count': 5, 'changes': 'All five tables unchanged'}
...
assert (root/'notes/determinatives-as-nouns-review-2026-09-12-modifier-series.md').read_bytes() == (snap/'author-review.md').read_bytes()
...
pages = [p for p in Path('/tmp/dnoun-bounded-repairs-layout.txt').read_text().split('\f') if p.strip()]
...
for label, source in [('before',before),('after',after)]:
    ...
    path = Path('/tmp/dnoun-bounded-repairs-count-'+label+'.tex'); path.write_text(rules+'\n'+body)
...
assert counts['before']['total'] == 9594
checks['texcount'] = counts
checks['net_words_added'] = counts['after']['total']-counts['before']['total']
checks['review_scope'] = 'Bounded author-directed repairs: approximative rationale for hardly, explicit constituency contrast, independent-genitive interpretation and agreement, relative-postmodifier scope, unsupported uptake-footnote removal'
(root/'notes/passes/2026-09-12-bounded-repairs-checks.json').write_text(json.dumps(checks,indent=2,ensure_ascii=False)+'\n')
(root/'notes/passes/2026-09-12-bounded-repairs.diff').write_text(''.join(difflib.unified_diff(before.splitlines(True),after.splitlines(True),fromfile='before/determinatives-as-nouns.tex',tofile='after/determinatives-as-nouns.tex')))
