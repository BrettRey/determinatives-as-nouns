...
\begin{table}[!htbp]
\centering\small
\caption{Representative profiles. Functions concern phrases; internal and peripheral modification concern Nom and NP respectively. Interrogative and relative forms follow \textit{CGEL}'s inventory.}\label{tab:existing}
\setlength{\tabcolsep}{4pt}
...
The strongest adjectival connection is concentrated in \mention{few}, \mention{many}, \mention{much}, and \mention{little}. They inflect for grade, take degree modifiers such as \mention{very} and \mention{so}, and license comparative \mention{than}-complements. These properties form a connected profile. Predicative use extends to \mention{several}, but the broader adverbial pattern involves approximative and focusing modifiers, analysed below at the NP periphery. Even the gradable forms have distinct syntax: \mention{so many mistakes} contrasts with \ungram{\mention{so numerous mistakes}} \citep[392--396, 431--432, 539--540]{huddleston2002}.

The case for inclusion rests on this distribution of connections. Determinatives share argument and determiner functions with all three noun subcategories, referential contrasts with pronouns, and number, genitive marking, and partitive constructions with established parts of Noun. Approximative and focusing modification reuses independently established NP grammar (§\ref{sec:payne}). The localized degree-modifier, grade, and complementation pattern supplies an adjectival counterweight. I give greater weight to the recurring nominal profile, while preserving that connected set of distinctive properties within the determinative subcategory.
...
The proposed analysis gives \mention{mine} ordinary Head in both anaphoric and predicative uses. Its genitive form expresses a relation to the speaker; context can supply the understood nominal description, as it can with independent \mention{few}. The \mention{my}/\mention{mine} alternation remains a form-selection condition, parallel to \mention{no}/\mention{none}.
...
Internal differences & Internal degree AdvP on \mention{few}, \mention{many}, \mention{much}, \mention{little}; approximatives after Det with independent cardinals. Compound restrictor order and non-recursion retained. & Same internal permissions, stated for primary D; same peripheral NP modification and compound conditions. & DP and Nom separate modifier domains. Same compound restrictor order and non-recursion. \\
...
The ordinary-Head accounts share peripheral NP rules for approximative and focusing modifiers. Their internal permissions cover the degree series on the four gradable quantifiers and approximatives on externally determined independent cardinals (§\ref{sec:payne}). D-noun states these permissions within Noun. The localized degree series accompanies grade and comparative complementation; the broader adverbial pattern fits NP structure. Generalizing NP degree modification outside nominal constructions carries a further cost for both accounts, extending the already qualified attributive-AdjP restriction with its \mention{a lot} exception (§\ref{sec:enough}).
