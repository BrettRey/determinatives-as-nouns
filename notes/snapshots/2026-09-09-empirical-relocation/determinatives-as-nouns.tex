...
I first distinguish the competing analyses, then establish what the existing noun subcategories share and how determinatives compare. Independent uses, partitives and modification identify the constructions to be explained. The corpus documents these uses; the feature-matrix reanalysis reassesses evidence for a primary determinative--pronoun boundary. The matched fragment in §\ref{sec:fragment} holds the judgments and lexical restrictions fixed while varying taxonomy and headedness. The comparison concerns whether shared nominal structure is better stated for Noun or licensed across categories, not whether only the D-noun analysis can cover the examples.
...
Saturation therefore clarifies the distributional comparison without independently selecting either internal structure. Both fusion and ordinary headedness accommodate syntactic completeness.
...
Fusion groups \mention{the lucky few} with \mention{the idle rich}, whereas ordinary headedness groups it with \mention{the lucky survivors}. Ordinary headedness instead unifies \mention{few} across its bare and externally determined uses, without alternating Det--Head and Mod--Head. Neither account gains coverage from this example alone. The comparison is between sharing a fusion construction across D and adjective, and sharing ordinary nominal headedness across determinative constructions.
...
This division extends to \mention{someone} and \mention{everybody}: the compound family licenses nominal argument use and post-head restrictors, while each base supplies its own premodifier permissions. It doesn't follow that \mention{hardly} modifies every compound.
...
This structural preference leaves the taxonomic question open: a separate-D grammar can also permit ordinary determinative heads. The pronoun precedent favours inheritance through Noun, while separate D preserves the modifier contrast as a primary-category distinction. Section~\ref{sec:fragment} compares both headedness choices under both taxonomies, making explicit what the distributional pattern leaves undecided. Before that comparison, §\ref{sec:empirical} examines what the empirical record establishes.

\section{What the empirical record establishes}\label{sec:empirical}

The corpus inventory below documents constructions discussed in §\ref{sec:evidence}. The feature-matrix reanalysis addresses a different question: what distributional differences establish about the determinative--pronoun boundary.
...
I inventory four CGELBank datasets containing 220 sentence trees and 3,390 lexical nodes with overt text. Of those nodes, 387 are annotated D. Appendix~\ref{app:corpus} gives the source revision, selection criteria and extraction procedure; an accompanying concordance preserves the sentences and their identifiers. Table~\ref{tab:corpus} compares selected forms by the local function of their projections. These are counts of annotated uses, not estimates of a lexeme's capacity for independent use.
...
\z\z
In (a), \mention{both} refers to the two Honda models. In (b), \mention{politicians or lawyers} supplies a domain for \mention{many} and \mention{none}. These attestations illustrate independent uses whose interpretation depends on preceding discourse. Appendix~\ref{app:corpus} identifies the source sentences.

CGELBank also attests \mention{I need something reliable and good looking}; the concordance gives the sentence identifier. The adjectives follow the compound determinative \mention{something}, contrasting with premodification in the constructed example \mention{the lucky few} (§\ref{sec:modification}).
...
The inventory thus attests independent and internally expanded determinatives, but provides no representative productivity estimate or controlled test of the modifier contrasts. The constructed examples in §\ref{sec:evidence} remain grammatical judgments.

\subsection{Distributional separation and taxonomic rank}

\textcite{reynolds2021} compared 73 determinative forms and 65 pronoun forms using a binary feature matrix: each row represents a form, and each column records a morphological, phonological, semantic or syntactic property. The matrix contains no common or proper nouns, so it can compare the two supplied categories but can't test nounhood directly.

The reanalysis uses two procedures. DISCO compares the supplied determinative--pronoun partition with shuffled partitions; k-groups searches for clusters without using the supplied category assignments. DISCO indicates separation between the supplied groups, but agreement between clustering and \textit{CGEL}'s classification depends on initialization and feature selection. Within the syntactic feature set, removing four columns that explicitly encode analytical decisions substantially reduces agreement. Appendix~\ref{app:matrix} gives the numerical results, compares the reported and publicly available feature counts, and explains the permutation comparison.
...
The corpus attests independent and internally expanded uses. The matrix reanalysis supports distributional separation from pronouns, with clustering agreement sensitive to initialization and feature selection. That separation doesn't determine taxonomic rank. Within Noun, coordination remains provisional: the fragment's judgments don't distinguish it from Hudson's nesting.
...
\subsection{Corpus inventory}\label{app:corpus}
...
\subsection{Feature matrix and sensitivity checks}\label{app:matrix}
