...
NPs already modify adjectives in \mention{a great deal better}. But \textit{CGEL} excludes \ungram{\mention{some a great deal better proposals}} while permitting \mention{some much better proposals} \citep[551--552]{huddleston2002}. Giving \mention{much} NP status requires restating that restriction on NP modifiers in attributive adjective phrases (AdjPs). This commitment follows the choice of NP projection for degree uses (§\ref{sec:enough}), rather than ordinary headedness alone.
...
Giving degree determinatives NP projection requires identifying the NPs permitted in attributive AdjPs while preserving exclusions such as \ungram{\mention{some a great deal better proposals}}. This revises an existing, already qualified restriction. All three NP-projecting implementations share that commitment. It counts in favour of retaining DP in degree uses, without establishing a preference for fusion or a separate primary D category.
...
\begin{table}[H]
\centering\small
\caption{Four implementations compared. All retain the lexical and constructional restrictions and adjectival fusion. The degree-use commitment follows NP projection. Cardinal and \mention{one} groupings are consequences of the categorization.}\label{tab:economy}
...
\midrule
D-noun, ordinary Head & Nom--NP with Head alone across bare, partitive, and externally determined uses. & Peripheral NP premodifiers; head-specific internal modifiers. Degree NPs require a revised attributive restriction. & D within Noun. Cardinal uses and the three \mention{one} lexemes are grouped there. \\
...
Separate D, fused Head & DP fills Det--Head or Mod--Head in nominal structure. & DP--Nom separates premodifiers and postmodifiers. Degree DPs retain \textit{CGEL}'s contrast with NPs. & Same primary-category divisions as separate D with ordinary Head. \\
