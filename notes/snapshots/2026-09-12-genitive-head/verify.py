...
root = Path.cwd()
snap = root / 'notes/snapshots/2026-09-12-genitive-head'
before = (snap / 'determinatives-as-nouns.tex').read_text()
after = (root / 'determinatives-as-nouns.tex').read_text()
review = root / 'notes/determinatives-as-nouns-review-2026-09-12-genitive-head.md'
md = review.read_text()
...
assert (root/'notes/determinatives-as-nouns-review-2026-09-12-head-consistency.md').read_bytes() == (snap/'author-review.md').read_bytes()
...
pages = [p for p in Path('/tmp/dnoun-genitive-head-layout.txt').read_text().split('\f') if p.strip()]
...
for label, source in [('before',before),('after',after)]:
    ...
    path = Path('/tmp/dnoun-genitive-head-count-'+label+'.tex'); path.write_text(rules+'\n'+body)
...
assert counts['before']['total'] == 9338
checks['texcount'] = counts
checks['net_words_added'] = counts['after']['total']-counts['before']['total']
checks['review_scope'] = 'Author rejects differential treatment of mine and few: ordinary Head for anaphoric and predicative mine, with interpretation and form-selection distinctions preserved'
(root/'notes/passes/2026-09-12-genitive-head-checks.json').write_text(json.dumps(checks,indent=2,ensure_ascii=False)+'\n')
(root/'notes/passes/2026-09-12-genitive-head.diff').write_text(''.join(difflib.unified_diff(before.splitlines(True),after.splitlines(True),fromfile='before/determinatives-as-nouns.tex',tofile='after/determinatives-as-nouns.tex')))
