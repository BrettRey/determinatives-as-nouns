...
old_review = (base / 'notes/snapshots/2026-09-08-rd-and-table-revision/determinatives-as-nouns-review-2026-09-08.md').read_text()
...
def figure(m):
    ...
    for i, t in enumerate(trees):
        ...
        if r'\draw' in t:
            ...
            src = next(j for j, row in enumerate(rows) if 'Obj: NP' in row)
            dst = next(j for j, row in enumerate(rows) if 'Det–Head: DP' in row)
...
out = base/'notes/determinatives-as-nouns-review-2026-09-09.md'
out.write_text(header + md)
assert tree_count == 11 and table_count == 7
