...
root = Path.cwd()
snap = root / 'notes/snapshots/2026-09-12-modifier-series'
before = (snap / 'determinatives-as-nouns.tex').read_text()
after = (root / 'determinatives-as-nouns.tex').read_text()
review = root / 'notes/determinatives-as-nouns-review-2026-09-12-modifier-series.md'
md = review.read_text()
...
assert [i for i,(a,b) in enumerate(zip(old_trees,new_trees)) if a!=b] == []
checks['trees'] = {'count': 11, 'changed_tree': 'All eleven trees unchanged, including the corrected NP layer in Figure 6', 'markdown': 'Unary branches rendered vertically; fusion links retained'}

old_schemata = re.findall(r'\\\[.*?\\\]', before, re.S)
new_schemata = re.findall(r'\\\[.*?\\\]', after, re.S)
assert len(new_schemata) == 4
expected = old_schemata
assert expected == new_schemata
checks['schemata'] = {'count': 4, 'change': 'All four schemata unchanged; attachment conditions made explicit in adjacent prose'}
...
new_tables = re.findall(r'\\begin\{table\}.*?\\end\{table\}', after, re.S)
assert [i for i,(a,b) in enumerate(zip(old_tables,new_tables)) if a!=b] == [1,4]
checks['tables'] = {'count': 5, 'changes': 'Table 2 splits internal and peripheral modification; Table 5 updates internal permissions; three other tables unchanged'}
...
assert (root/'notes/determinatives-as-nouns-review-2026-09-12-genitive-head.md').read_bytes() == (snap/'author-review.md').read_bytes()
...
assert not re.search(r'(?:Citation|Reference) .+ undefined|There were undefined|Overfull|Underfull|Float too large|Rerun to get|Please rerun|^!', log, re.M)
assert not re.search(r'\b(?:TODO|FIXME|VERIFY)\b|\?\?',after)
prose_without_urls = re.sub(r'https?://[^}]+','',after)
assert not re.search(r'\b(?:class|classif\w*|reclassif\w*)\b',prose_without_urls.replace(r'\mention{almost the whole class}', ''),re.I)
pages = [p for p in Path('/tmp/dnoun-modifier-series-layout.txt').read_text().split('\f') if p.strip()]
...
for label, source in [('before',before),('after',after)]:
    ...
    path = Path('/tmp/dnoun-modifier-series-count-'+label+'.tex'); path.write_text(rules+'\n'+body)
...
assert counts['before']['total'] == 9334
checks['texcount'] = counts
checks['net_words_added'] = counts['after']['total']-counts['before']['total']
checks['review_scope'] = 'Author-directed modifier-series distinction: peripheral approximative/focusing NP modification; internal degree series with four gradable quantifiers in both uses; cardinal exception and separate completeness permissions retained'
(root/'notes/passes/2026-09-12-modifier-series-checks.json').write_text(json.dumps(checks,indent=2,ensure_ascii=False)+'\n')
(root/'notes/passes/2026-09-12-modifier-series.diff').write_text(''.join(difflib.unified_diff(before.splitlines(True),after.splitlines(True),fromfile='before/determinatives-as-nouns.tex',tofile='after/determinatives-as-nouns.tex')))
