...
\begin{abstract}
I argue that English determinatives form a fourth subcategory of Noun, alongside common nouns, proper nouns and pronouns. The argument extends the distributional grounds on which \textit{The Cambridge grammar of the English language} already includes pronouns within Noun despite their distinctive inflection and restricted dependents. Determinatives likewise combine systematic nominal uses with a distinctive internal profile. Comparisons of independent uses, partitives and modification support ordinary nominal headedness while preserving lexical restrictions, including those of articles. An explicit grammatical fragment shows how this analysis extends existing noun-phrase structure and reduces the phrase types admitted in determiner function. Corpus attestations and a reanalysis of published feature data delimit the empirical support. Distributional differences warrant retaining determinative and pronoun as distinct subcategories; they don't by themselves establish a primary-category boundary or decide between coordinate and nested membership within Noun.
\end{abstract}
...
I first distinguish the competing analyses, then establish what the existing noun subcategories share and how determinatives compare. Independent uses, partitives and modification supply the positive case, with predicative restrictions considered separately. Corpus examples and a reanalysis of published determinative--pronoun data establish the empirical scope. A final grammatical fragment makes the structural gain and its costs explicit.
...
The positive distributional test follows the same starting point as \textit{CGEL}'s pronoun argument. Can a range of determinatives head phrases in subject, object and complement-of-preposition positions without another overt nominal head? The pattern should recur across lexemes and appropriate discourse contexts. This tests systematic nominal use before assigning its internal structure. The examples in §\ref{sec:evidence} compare all four proposed subcategories in the same positions.
...
Figure~\ref{fig:genitive} illustrates the proper-noun case. The genitive NP \mention{Kim's} functions as Det, while \mention{book} ultimately heads the larger NP. This existing configuration provides the comparison for the determinative analysis that follows.
...
Saturation thus establishes the availability of complete argument expressions without an overt determiner or common noun. The preference for ordinary headedness comes from combining this fact with the established noun comparison and the internal structures below. It doesn't depend on treating the absence of an overt antecedent as proof against every silent-noun account.
...
The constructed examples \mention{the remaining three} and \mention{the selected two} broaden the comparison beyond \mention{few}. On the intended readings, the expressions denote the people remaining or selected, and the cardinal specifies their number. The modifier occurs between the article and the quantificational head, as in \mention{the lucky few}. This pattern therefore calls for an account extending beyond one lexical item.
% Constructed examples retained from the April 29 draft, commit 1f9973b.

The ordinary-Head analysis gives these expressions the same Det--Nom organization as \mention{the lucky survivors}, with an overt lexical head in each. A silent-noun analysis introduces another head; a conversion analysis assigns a common-noun use to the quantifier or cardinal. Those alternatives need a benefit that offsets the additional structure or category assignment. In the displayed constructions, the D-noun analysis captures the overt modifier--head pattern while preserving the quantificational item's subcategory.
...
The contribution of modification is consequently structural. It supplies positive parallels with ordinary nominal organization and broadens the lexical coverage of that comparison. The argument for a noun subcategory combines these parallels with independent distribution; it doesn't require every determinative to permit adjectival modification.
...
\subsection{The cumulative case for ordinary headedness}\label{sec:cumulative}

The comparisons favour ordinary headedness for the simple independent cases. First, nominal argument use recurs across the determinative inventory, so it calls for a category-level account. Second, those expressions are complete without an overt common noun, like ordinary pronoun- and proper-noun-headed NPs. Third, partitives and internally modified expressions locate an overt quantificational head in structures that can share ordinary nominal organization. The modifier and predicative restrictions specify the resulting subcategory's profile.

I take this combined pattern to support extending the existing Noun category. \textit{CGEL} already admits pronouns through shared nominal functions despite distinct inflection and dependents. Determinatives supply a further instance of that relation between common external behaviour and specific internal restrictions. On the D-noun analysis, the ordinary Head relation follows the same nominal sequence across all four subcategories, and lexical identity is preserved across dependent and independent uses.

Fusion can describe the same independent cases, and it remains needed elsewhere. The comparative advantage here is that a widespread use receives the structure already available for other nouns, with the restrictions retained. A separate-D account would earn its additional distinction by capturing a generalization lost under the extension, or by giving the constructions a better analysis. Section~\ref{sec:fragment} identifies what changes when the alternatives receive the same lexical permissions.
...
The separate-D grammar already needs nominal projection for common nouns, proper nouns and pronouns. For the determinative constructions it additionally needs a DP projection and a route from that projection to independent NP use. Abstracting from the same lexical conditions, those two schemata are:
\[
\begin{aligned}
\mathrm{DP}_{h} &\to (\mathrm{Mod}_{\mathrm{pre}})\quad \mathrm{Head}:\mathrm{D}_{h}\quad (\mathrm{Comp}) \\
\mathrm{NP}_{h} &\to \mathrm{Det\text{--}Head}:\mathrm{DP}_{h}\quad (\mathrm{Mod}_{\mathrm{post}})
\end{aligned}
\]
The first supplies phrases such as \mention{almost every}; the second gives independent \mention{some} its containing NP through fusion. In the ordinary-Head variant, the existing Nom and NP schemata cover these determinatives, with their lexical permissions unchanged. The partitive PP remains a post-head modifier: beside the fused head in NP in these schemata, and inside Nom in the ordinary-Head analysis.
...
A separate-D grammar could instead license D-headed NPs directly, through a rule permitting either N or D as lexical head. That alternative captures the shared projection while retaining the primary-category boundary. The choice then returns to the membership argument: why should systematic nominal projection warrant inclusion for pronouns but require cross-category licensing for determinatives? The differences that motivate the determinative subcategory remain available under either description.

The D-noun analysis extends an established superordinate category to capture that common profile. Its justification combines the existing grounds for nounhood with the structural comparison; it doesn't rest on counting category names. A separate-D account could still be preferable if the primary boundary captured enough additional grammatical generalizations to offset the shared projection. The modifier contrast below supplies a concrete case to weigh.
...
The proposal changes the scope of one broader generalization. A ban on internal AdvP premodifiers can no longer be stated for Noun as a whole, because \mention{hardly any} and \mention{almost every} now contain noun heads. The restriction has to distinguish determinatives from the other noun subcategories. This is the consequence of the modifier contrast in \textcite[40--42]{payne2010}; the nounhood argument can't simply ignore it.
...
Approximative \mention{or so} supplies a smaller constructional comparison: \mention{fifteen or so minutes} and \mention{ready by Tuesday or so}. \textit{CGEL} identifies the numerical pattern as coordination \citep[431, n.~47]{huddleston2002}. The idiom spans cardinal and naming expressions in these constructed examples. Its dependence on a relevant scale, here quantity or time, limits its value as a category test. It is compatible with the broader nominal analysis without independently establishing it.
...
I argue for determinative as a fourth noun subcategory in English. \textit{CGEL} already admits pronouns within Noun on the basis of their phrase functions despite differences in inflection and dependents. Determinatives show the same kind of relation between systematic nominal uses and a distinctive internal profile. The case for inclusion follows that established standard and develops it through independent uses, partitives and modification.

Ordinary headedness gives the simple independent cases the nominal structure already needed elsewhere. Determinatives retain their lexical identity, and the articles retain their paradigmatic membership with independent use restricted. Predicative and modifier differences remain explicit. The analysis also removes the DP/NP phrase-type disjunction within determiner function, preserving feature selection and PP determiners. The cost is a narrower scope for some generalizations previously stated for Noun as a whole.

The corpus and matrix analyses establish the empirical scope of the comparison. They support attested nominal uses and distinguish coded determinative and pronoun profiles without fixing taxonomic rank. Coordinate subcategories preserve that local distinction within the existing nominal architecture; additional shared syntactic generalizations could favour Hudson's nesting. The central proposal is to extend to determinatives the nounhood that the framework already grants to other grammatically distinctive subcategories.
