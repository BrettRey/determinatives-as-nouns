...
These analyses accompany the working paper as separate supplements. Independent numerical review is complete: the auditor reran all matrix and corpus scripts, reproduced their outputs byte for byte, and separately recomputed the DISCO components and corpus counts. The complete audit is in `../reviews/review-board-20260907-rebuild/corpus.md`; a separate numerical check is in `../reviews/review-board-20260907-rebuild/numbers-second-model.md`. No preregistration, independent judgment study, representative corpus sample or held-out category-classification experiment is claimed.
