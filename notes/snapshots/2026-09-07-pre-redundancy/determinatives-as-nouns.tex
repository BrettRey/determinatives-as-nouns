...
\begin{abstract}
English determinatives such as \mention{some}, \mention{this} and \mention{many} occur both with a following common noun and independently. I argue that the determinative category belongs within Noun, alongside common noun, proper noun and pronoun. The proposal gives independent determinatives ordinary nominal structure and preserves their lexical identity across uses. Worked analyses distinguish lexical classification, phrase headedness and grammatical function. A matched descriptive fragment states how \mention{some}, \mention{every} and the articles are licensed, and separates the effect of nominal classification from the replacement of determiner--head fusion. The resulting economy is structural: the displayed independent uses share nominal projection already needed elsewhere, while their lexical restrictions remain. A reproduction of the determinative--pronoun feature matrix confirms distributional separation but finds initialization and feature sensitivity in the clustering result. A reproducible CGELBank inventory provides attested contexts for the grammatical comparison. Together, the analyses support a nominal reconstruction of CGEL's determinative category. The coordinate arrangement preserves its determinative/pronoun distinction; the same fragment also identifies what remains compatible with Hudson's broader pronoun analysis.
\end{abstract}
...
The descriptive issue is where a grammar states the shared structure of these expressions. Under the proposal, independent determinatives use the nominal projection already needed for the other noun subclasses. Their distinctive dependents and lexical gaps remain explicit. The contribution is a worked reconstruction of that analysis, including a fragment that separates the consequences of nominal classification from those of replacing fusion.
...
The alternative to a separate primary D needn't preserve a unified determinative subclass. \textcite{spinillo2004reconceptualising} proposes redistribution among existing categories, retaining \mention{the}, \mention{a} and \mention{every} as an expanded article category. That proposal makes the restricted members a separate class; the fourfold analysis retains them inside the wider determinative subclass. The difference concerns the internal cohesion of that subclass, not merely whether to remove a primary D category.

\textcite{vaneynde2003determiner} likewise rejects a separate determiner category, but assigns the relevant expressions to adjective or noun on morphological and agreement evidence, chiefly from Italian and Dutch. His analysis separates lexical category from the features governing determination. It shows why a noun-headed NP account needn't unify every determining expression as nominal. The present English proposal needs its own distributional case; the absence of the cited inflectional patterns in English can't refute his analysis of other languages.
...
The relevant alternative is the developed account of fusion in \textcite{Payne2007}, not simply a special label attached to independent words. Their six defining properties concern joint realization of two functions, locality of the fused relation, adjacency, the categories of the fused constituent and its containing phrase, and two correspondences with a non-fused construction. These properties constrain the analysis as a whole. Replacing a fused relation requires identifying which of its work is preserved elsewhere.

For simple independent determinatives, the ordinary-Head analysis rejects joint realization: \mention{some} is Head, with no simultaneous Det function. The locality and adjacency conditions consequently have no two-function relation to constrain. The nominal category supplies the path from N through Nom to NP. Lexical identity across dependent and independent uses preserves the counterpart correspondence, but the two occurrences fill different functions. This is a redistribution of descriptive work, not a finding that the fusion properties are internally inconsistent.

\textcite{Payne2007} also explicitly ties the exclusion of independent \mention{every} and the form \mention{none} to lexical differences in fusion licensing. The nominal account still needs those differences, now stated as permissions for independent use and form selection. Its advantage can't be a claim that fusion overlooks them. Section~\ref{sec:fragment} holds those permissions fixed when comparing the structures.

Adjectival fusion is retained. In \mention{the rich}, \mention{rich} remains adjectival and realizes fused Mod--Head within Nom; the external article fills Det. In \mention{the idle rich}, the adjective \mention{idle} modifies that nominal. The proposal makes no general conversion of such adjectives into nouns. The broader independent distribution of determinatives motivates testing ordinary nominal projection for that category, while the restricted adjectival constructions retain their separate analysis. This is a category-level comparison, not a claim that the word order of \mention{the lucky few} by itself excludes fusion.

Compound determinatives present a further challenge. For \mention{hardly anyone present}, \textcite{Payne2007} uses the D projection to license the pre-head adverb and NP structure to license the post-head adjective. An ordinary nominal account instead needs the determinative subclass and the compound construction to retain that distinction inside Noun. The simple-head fragment below doesn't remove the need for this compound-specific description. Its economy is correspondingly limited to the constructions it actually analyses.
...
Adjectival postmodification is attested in \mention{I need something reliable and good looking}. Its full sentence identifier is retained in the concordance. This is an attestation of adjectival postmodification of a compound determinative. It isn't evidence for the specific premodification pattern in \mention{the lucky few}. Keeping that difference visible prevents a related attestation from being counted as confirmation of the disputed construction.
...
This matters particularly for the comparison with Hudson. \mention{Pronoun} has a broader extension in his analysis than in CGEL's matrix labels. Testing the CGEL determinative rows against the CGEL pronoun rows is therefore not a direct test of Hudson's broader category. His determiner terminology and the CGEL determinative inventory don't identify the same domain. A comparison of hierarchies must first align the relevant items and uses, then compare the generalizations that their groupings support.
...
The coordinate proposal has a particular advantage as a revision of CGEL: it retains the local determinative/pronoun distinction and extends the shared nominal analysis. No word must change between determinative and pronoun simply because it occurs independently. Nor must the broad determinative inventory first be repartitioned into narrower classes. These are commitments of the proposal, not proof that its hierarchy is the only one compatible with the data. The preservation of an existing classification is an economy relative to that descriptive starting point.

An intermediate category would be favoured if it enabled additional shared restrictions to be inherited with fewer exceptions than the coordinate analysis. Coordination would be favoured if the proposed intermediate properties repeatedly required separate statements for its constituent groups, while the shared properties belonged at Noun anyway. The matched fragment in §\ref{sec:fragment} shows that its inherited permissions are compatible with either hierarchy. The matrix adds evidence about differences between lexical profiles, but no property requiring the intermediate category.
...
The allowance for restricted members is compatible with the existing nominal system. A dependent genitive pronoun such as \mention{my} is nominal in CGEL's analysis even though it doesn't have the independent distribution of \mention{mine} \citep[470--471]{huddleston2002}. Likewise, the common-noun projection has conditions on bare singular count arguments. Membership in Noun therefore doesn't entail an unrestricted one-word NP. The question is whether the article restrictions are reasonably localized within the proposed subclass.

Under the fourfold account, a determiner use of \mention{the} contains an NP\textsubscript{D}, but that phrase isn't thereby licensed as a free-standing argument. Section~\ref{sec:fragment} states the restriction as a use permission: the article-headed projection is licensed in Det function, while an ordinary subject or object requires argument permission. The label NP by itself supplies neither permission.
...
A small fragment makes the division between taxonomy and structure explicit. It covers determination of common-noun nominals, ordinary subject and object uses, partitives, and the modifier contrasts already discussed. The three analyses compared below receive the same lexical permissions and the same constructed judgments. This is a comparison of descriptions of those cases, not a new judgment study or an exhaustive grammar of English.
...
For \mention{some apples}, \mention{some} projects a nominal NP whose Det permission and plural-count target requirement are satisfied. \mention{Apples} heads the outer nominal and NP. In \mention{Some left}, the same lexeme projects an NP whose argument permission is satisfied. \mention{Every apple} passes the corresponding Det and singular-count checks; ordinary independent \ungram{\mention{Every left}} fails the argument-permission check. \mention{The apple} and \ungram{\mention{The left}}, with \mention{left} as a verb, differ in the same way. No missing noun is needed to state that contrast.

The common-noun control involves a different condition. In \mention{a book}, the singular count head's requirement for determination is satisfied; bare \ungram{\mention{Book arrived}} leaves it unsatisfied. \mention{Books arrived} doesn't have that requirement. Both restrictions are needed: requiring determination for a singular count common noun doesn't itself block an article-headed NP from argument use. The latter restriction is the separate lexical permission in Table~\ref{tab:permissions}. The analogy with common nouns establishes that nominal projection can be restricted, not that all such restrictions are identical.
...
The first change removes a primary-category boundary; it doesn't remove fusion. The second replaces the fused relation in the displayed independent D constructions with the ordinary nominal projection. Fusion remains available for other constructions, including adjectival fused heads and anaphoric independent genitives. The fragment therefore supports a specific structural simplification: the independent D cases share an existing projection and Head relation. It doesn't establish the elimination of a grammatical mechanism or a reduction in the number of lexical restrictions.
...
A nested nominal account can inherit these permissions and projection rules too. Replacing the direct Noun-to-determinative inheritance relation with one through a broader pronoun category changes none of the fragment's judgments, provided that no incompatible restriction is added at the intermediate level. The fragment thus identifies an equivalence: these cases support the shared nominal treatment but leave the intermediate hierarchy open. Coordination preserves CGEL's determinative/pronoun distinction without requiring that intermediate category; a further shared generalization could still justify it.
...
The next comparison should extend the fragment to constructions that distinguish its analyses. Each account would need to state the additional inherited properties and test them against contexts beyond those used to formulate the description. A repeated generalization specific to pronouns and determinatives would support an intermediate superclass; generalizations extending across the nominal domain belong at Noun. A visually flatter hierarchy supplies no independent criterion.
