...
root = Path.cwd()
snap = root / 'notes/snapshots/2026-09-12-head-consistency'
before = (snap / 'determinatives-as-nouns.tex').read_text()
after = (root / 'determinatives-as-nouns.tex').read_text()
review = root / 'notes/determinatives-as-nouns-review-2026-09-12-head-consistency.md'
md = review.read_text()
...
assert [i for i,(a,b) in enumerate(zip(old_trees,new_trees)) if a!=b] == [9]
checks['trees'] = {'count': 11, 'changed_tree': 'Figure 6 ordinary-Head panel: hardly peripheral to NP, present internal to Nom; ten other trees unchanged', 'markdown': 'Unary branches rendered vertically; fusion links retained'}
...
checks['tables'] = {'count': 5, 'changes': 'Comparison caption and internal-differences cell revised; four other tables unchanged'}
...
assert (root/'notes/determinatives-as-nouns-review-2026-09-11-consolidated.md').read_bytes() == (snap/'author-review.md').read_bytes()
...
pages = [p for p in Path('/tmp/dnoun-head-consistency-layout.txt').read_text().split('\f') if p.strip()]
...
for label, source in [('before',before),('after',after)]:
    ...
    path = Path('/tmp/dnoun-head-consistency-count-'+label+'.tex'); path.write_text(rules+'\n'+body)
...
assert counts['before']['total'] == 9367
checks['texcount'] = counts
checks['net_words_added'] = counts['after']['total']-counts['before']['total']
checks['review_scope'] = 'Author-specified consistency repair: ordinary Head retained throughout determinatives; peripheral versus internal attachment fixed by construction; anaphoric genitive fusion explicit; abstract and comparative accounting aligned'
(root/'notes/passes/2026-09-12-head-consistency-checks.json').write_text(json.dumps(checks,indent=2,ensure_ascii=False)+'\n')
(root/'notes/passes/2026-09-12-head-consistency.diff').write_text(''.join(difflib.unified_diff(before.splitlines(True),after.splitlines(True),fromfile='before/determinatives-as-nouns.tex',tofile='after/determinatives-as-nouns.tex')))
