...
I adopt the general framework of \textit{The Cambridge grammar of the English language} (\textit{CGEL}; \citealt{huddleston2002}). Unlike \textit{CGEL}, though, I include determinatives within Noun. The claims concern synchronic English lexical categories.
...
In the D-noun analysis, determinatives inherit the nominal projection rules of Noun. A determinative heads a \term{nominal} (Nom), which heads an NP. Nom contains the head and its internal dependents, excluding an external determiner. Lexical restrictions govern which uses are available. The placement of determinative alongside pronoun is a further proposal, assessed in §\ref{sec:inheritance}.

Figure~\ref{fig:some} compares \textit{CGEL} with the D-noun analysis for \mention{take some apples} and \mention{take some}. Both keep \mention{some} in one lexical category across uses. In \textit{CGEL}, independent \mention{some} jointly fills determiner and head functions, a \term{fusion of functions} \citep[410--412]{huddleston2002}. In the D-noun analysis, \mention{some} heads its own NP through a Nom in both uses.
...
Earlier unifications include Palmer's grouping of determinatives with pronouns against the traditional adjectival classification, Postal's underlying article analysis of pronouns, and Sommerstein's underlying NP analysis of articles and pronouns \citep{palmer1924,postal1966,sommerstein1972}. Hudson places the words he calls determiners within pronoun and pronoun within noun \citep{hudson2004determiners,hudson2010wordgrammar}. Section~\ref{sec:related} compares these proposals; Table~\ref{tab:rivals} locates the taxonomic alternatives.
...
\textit{CGEL} also uses fusion in noun-headed structures. It analyses \mention{mine} as fused Det--Head when it stands for a possessed entity in an anaphoric context, but as pure Head in the predicative possessive use \mention{it's mine} \citep[410--411]{huddleston2002}. The D-noun analysis of independent determinatives extends the ordinary projection illustrated by \mention{she} and \mention{Kim}.
...
Table~\ref{tab:economy} locates the three accounts. Comparing the ordinary-Head accounts holds projection fixed while varying taxonomy. Comparing the separate-D accounts holds taxonomy fixed while varying headedness.\footnote{A nominal-D grammar could also stipulate fusion, replacing the fused DP with a determinative-headed NP. This would add a fused relation around an already available nominal projection. No property of the simple independent uses considered here requires that additional relation.}
