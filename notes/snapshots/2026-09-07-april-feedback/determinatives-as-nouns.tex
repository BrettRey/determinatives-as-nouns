...
\begin{table}[tb]
\centering\small
\caption{Selected taxonomic arrangements. A logical comparison is a possible arrangement used to locate the proposals discussed here. The Hudson row abstracts from differences in lexical inventory explained in the text.}\label{tab:rivals}
...
The corpus supplies attested examples; the matrix reanalysis confirms differences between coded determinative and pronoun profiles. The grammatical fragment shows how projection rules can be shared while retaining local restrictions. The coordinate classification preserves \textit{CGEL}'s determinative/pronoun distinction. An advantage over Hudson's nested classification would depend on further generalizations that distinguish the two hierarchies.
