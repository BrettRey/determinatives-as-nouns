...
baseline = (base / 'notes/snapshots/2026-09-12-package-clarification/determinatives-as-nouns.tex').read_text()
baseline = baseline.replace(r'(\mathrm{Mod}_{\mathrm{post}})', r'\mathrm{Mods}_{\mathrm{post}}')
baseline = baseline.replace('Both ordinary-Head accounts:', 'The three NP-projecting accounts:')
...
def tree_text(tree):
    out, nodes = [], []
    def emit(node, prefix, marker='', parent=None):
        label, children = node
        row = len(out)
        out.append(prefix + marker + label)
        nodes.append({'row': row, 'label': label, 'parent': parent})
        node_id = len(nodes)-1
        child_prefix = prefix + ('    ' if marker == '└── ' else '│   ' if marker else '')
        if len(children) == 1:
            out.append(child_prefix + '│')
            emit(children[0], child_prefix, parent=node_id)
        else:
            for i, child in enumerate(children):
                emit(child, child_prefix, '└── ' if i == len(children)-1 else '├── ', node_id)
    emit(tree, '')
    return out, nodes
...
def figure(m):
    ...
    for i, t in enumerate(trees):
        rows, nodes = tree_text(parse_tree(t))
        if r'\draw' in t:
            fusion = next(n for n in nodes if n['label'] in ('Det–Head: DP', 'Mod–Head: DP'))
            dst = fusion['row']
            src = nodes[nodes[fusion['parent']]['parent']]['row']
...
math_blocks[0] = math_blocks[0].replace('Both ordinary-Head accounts:', 'The three NP-projecting accounts:')
assert 'NP_h → (Det)' in math_blocks[1]
math_blocks[1] = math_blocks[1].replace('NP_h → (Det)', 'NP_h → (Mod_periph) (Det)')
math_blocks = [block.replace('(Mod_post)', 'Mods_post') for block in math_blocks]
math_count = 0
def display(m):
    global math_count
...
body = body.replace(r'$\mathrm{Mod}_{\mathrm{periph}}$', r'\texttt{Mod\_periph}')
body = body.replace(r'$\mathrm{Mods}_{\mathrm{post}}$', r'\texttt{Mods\_post}')
...
md = re.sub(r'(?m)(\|[^\n]+\n(?:\|[^\n]+\n)+)\n(Table [^\n]+)', r'\2\n\n\1', md)
md = md.replace('*— ', '* — ').replace('On the left,', 'In the first tree,')
...
out = base/'notes/determinatives-as-nouns-review-2026-09-12-package-clarification.md'
...
md = md.replace('](run:matrix-audit.pdf)', '](../matrix-audit.pdf)').replace('](run:corpus-documentation.pdf)', '](../corpus-documentation.pdf)')
if out.exists():
    assert not re.search(r'\{(?:>>|\+\+|~~|--)', out.read_text()), 'Preserve reviewed copy'
out.write_text(header + md)
assert tree_count == 11 and table_count == 6
