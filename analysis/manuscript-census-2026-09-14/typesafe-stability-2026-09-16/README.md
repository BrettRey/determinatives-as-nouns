...
[confidence-histogram.pdf](confidence-histogram.pdf) (`confidence_histogram.py`) shows all 1,974 construction answers across the three phrasings, one bin per value: the API rounds every probability and confidence to two decimals (83 distinct confidence values), so coarser bins alias into a sawtooth. There is no gap at 0.5: the 0.45 to 0.55 band holds 14 percent of answers on a broad plateau running from about 0.42 to 0.66, and the taxonomic option is over-represented there (37 percent of that band against 28 percent overall), which is the two-way split between a specific construction and the escape. The mass sits at the top: 29 percent of answers are at 0.90 or above and 0.99 is the commonest single value. Confidence tracks the top-option probability within about 0.05 throughout.
