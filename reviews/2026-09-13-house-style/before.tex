...
The ordinary-Head analysis uses the same chain in bare, partitive, and externally determined expressions: the word heads Nom, and Nom heads NP. This also applies to independent genitives such as \mention{mine}. The taxonomic and structural choices complement each other: the nominal profile supports grouping determinatives with nouns, and that grouping satisfies the proposed grammar's category condition on lexical Heads in Nom.
